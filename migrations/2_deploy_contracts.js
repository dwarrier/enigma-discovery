...
async function deploySecretContract(config) {
  ...
  if (result) {
    fs.writeFile(
      path.join("test/", config.filename.replace(/\.wasm$/, ".txt")),
      scTask.scAddr,
      "utf8",
      function(err) {
        if (err) {
...
module.exports = async function(deployer, network, accounts) {
  ...
  deployer.deploy(Sample).then(function() {
    console.log(
      `Smart Contract "Sample.Sol" has been deployed at ETH address: ${Sample.address}`
    );
    return;
  });
  ...
  const addressMillionairesProblem = await deploySecretContract(
    configMillionairesProblem
  );
  console.log(
